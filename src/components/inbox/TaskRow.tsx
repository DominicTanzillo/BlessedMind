  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-xl transition group ${
      ...
      <div className={`flex items-center gap-1 transition ${task.starred ? '' : 'opacity-0 group-hover:opacity-100'}`}>
        ...
        {!task.completed && (
          <button
            onClick={() => task.starred ? onUnstar(task.id) : onStar(task.id)}
            className={`p-1 rounded transition ${
              task.starred
                ? 'text-sage-500 hover:text-terracotta'
                : 'text-stone-300 hover:text-sage-500'
            }`}
            title={task.starred ? 'Remove priority boost' : 'Boost priority'}
          >
            <svg className="w-4 h-4" fill={task.starred ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
